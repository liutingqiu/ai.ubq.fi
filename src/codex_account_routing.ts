...
const resolveAdmissionRefreshGeneration = async (account: RoutingAccount, pool: CodexAuthPoolState, routingValue: unknown): Promise<number | null> => {
  ...
  if (currentSlot !== null && (currentSlot.account_id_hash !== account.accountIdHash || currentSlot.credential_version !== account.credentialVersion || currentSlot.invalid_credential_version === account.credentialVersion))
    return null;
